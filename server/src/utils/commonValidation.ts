...
interface CommonArgsTypes {
  element: string;
  required: boolean;
  minLength?: number;
  maxLength?: number;
}
interface NumberArgsTypes {
  element: string;
  required: boolean;
  minValue?: number;
  maxValue?: number;
...
export const createNumberValidation = ({
  ...
  minValue = 0,
  maxValue = 100,
}: NumberArgsTypes): Joi.NumberSchema => {
...
export const createEnumValidation = (el: string) =>
  Joi.string()
