import Joi from 'joi';
import { createCommonStringValidation, createEnumValidation, createNumberValidation, inputIdSchema, objectIdPattern, } from '../../utils/commonValidation';
...
export const batchUpdateSchema = Joi.array().items(Joi.object({
    _id: Joi.string().pattern(objectIdPattern).required().messages({
        'string.pattern.base': '"_id" must be a valid ObjectId',
        'string.base': '"_id" must be a string',
        'any.required': '"_id" is required',
